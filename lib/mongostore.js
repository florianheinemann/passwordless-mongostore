...
/**
 * Stores a new token incl. the linked user details or updates a record
 * for a certain user if a token already exists. Hence, a user can only
 ...
 * error or as callback() if the token was successully stored / updated
 */
MongoStore.prototype.remove = function(token, callback) {
	if(!token || !callback) {
		throw new Error('TokenStore:remove called with invalid parameters');
	}
	this._get_collection(function(collection) {
		collection.remove( { 'hashedToken': token}, {w:1}, function(err, result) {
			if(err) {
				callback(err);
			} else {
				callback();
