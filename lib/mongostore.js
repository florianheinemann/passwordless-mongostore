...
MongoStore.prototype.storeOrUpdate = function(token, uid, msToLive, originUrl, callback) {
	...
	this._get_collection(function(collection) {
		bcrypt.hash(token, 10, function(err, hashedToken) {
			...
			// Insert or update
			collection.updateOne( {'uid': uid}, {'$set': newRecord}, {w:1, upsert:true}, function(err, result) {
				if(err) {
					callback(err);
				} else {
					callback();
...
MongoStore.prototype._connect = function(callback) {
	...
	if(self._db) {
		...
		MongoClient.connect(self._uri, self._options, function(err, client) {
			if(err) {
...
MongoStore.prototype._set_index = function(collection, callback) {
	...
	collection.createIndex( { 'uid': 1 }, { unique: true }, function(err, index) {
		...
		collection.createIndex( { "ttl": 1 }, { expireAfterSeconds: 0 }, function(err, index) {
			if(err) {
